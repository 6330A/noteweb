...
[视频地址](https://www.bilibili.com/video/BV1ke4y1w7yn?p=2&spm_id_from=pageDriver&vd_source=b9f16feb6ff7836e90c4ba95657422ea)

:pencil:静态代理：编译时将接口、实现类、代理类编译为字节码文件，然后加载到JVM中

:pencil:动态代理：运行时动态生成类字节码，并加载到JVM中
...
```java
//1.Class类静态方法forName，参数是全类名
Class clazz1 = Class.forName("com.nenu.reflectdemo.Student");
...
```java
// 获取字节码文件对象
Class clazz = Class.forName("com.nenu.reflectdemo.Student");
...
//获取整体的字节码文件对象
Class clazz = Class.forName("com.nenu.myreflect.Student");
//获取空参的构造方法对象并使用newInstance方法创建对象
Constructor con1 = clazz.getConstructor();
Student stu1 = (Student) con1.newInstance();

// 获取有参构造，这个是private修饰的构造方法，Declared + setAccessible
Constructor con2 = clazz.getDeclaredConstructor(String.class, int.class);
con2.setAccessible(true);
Student stu2 = (Student) con2.newInstance("Tom", 123);
...
Field f1 = clazz.getField("gender");
Field f2 = clazz.getField("address");
...
```java
Student student = new Student("Tom", 12, "boy", "America");
Class clazz = Class.forName("com.nenu.myreflect.Student");
...
- **无视修饰符访问类**（暴力反射）中的内容。但是这种操作在开发中一般不用，都是框架底层来用的。

- 反射可以跟**配置文件**结合起来使用，动态的创建对象，**动态的调用**方法。
 ...
 * 程序编译后产生的.class文件没有泛型约束，这种现象我们称为
...
public class Reflect {
    public static void main(String[] args) throws ReflectiveOperationException {
        ...
        addMethod.invoke(array, "this is a string");
...
配置文件中的信息：
classname=com.nenu.reflectdemo.Student
methodname=sleep
```
...
public class Main {
    public static void main(String[] args) {
        /**
         * 动态代理的实现
         * 1.使用 java.lang.reflect 包下的 Proxy 代理类实例化代理对象
         * 2.调用 Proxy 的 newProxyInstance 方法,提供三个参数
         * 3.第一个参数就是一个类加载器
         * 4.第二个参数就是目标类实现的接口方法数组 new Class[]{Service.class,} 可以多个
         * 5.第三个参数是一个实现了 InvocationHandler 接口的实例,实现其中的invoke方法即可
         */
        ServiceImpl target = new ServiceImpl();   // 目标类对象作为下面参数

        // 一般这个代理proxy会用一个类封装, 返回代理对象即可,提供目标对象参数也就是targer
        Service proxy = (Service) Proxy.newProxyInstance(
                Main.class.getClassLoader(),
                new Class[]{Service.class},
                new InvocationHandler() {  // 可以用 lambda 表达式简化
                    @Override
                    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
